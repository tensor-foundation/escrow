...
import type {
  PoolConfigArgs,
  DetachPoolFromMarginAsyncInput,
} from "@tensor-foundation/escrow";
import {
  fetchPool,
  getDetachPoolFromMarginInstructionAsync,
} from "@tensor-foundation/escrow";
...
async function detachEscrowFromPool(poolAddress: string) {
  ...
  const pool = await fetchPool(rpc, address(poolAddress)).catch(() => {
    throw new Error(
      `Rpc couldn't fetch pool with address ${poolAddress}, please make sure the input is a valid pool address and that your RPC URL is correct!`,
    );
  });
  if (isNone(pool.data.margin)) {
    throw new Error(
      `${poolAddress} is not attached to shared escrow, can't detach!`,
    );
  }
  ...
  const detachPoolIx = await getDetachPoolFromMarginInstructionAsync(
    detachPoolFromMarginAsyncInput,
  );
